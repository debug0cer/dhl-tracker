...
const STATUS_LABELS = {
    ...
    arrived: 'Im Paketzentrum angekommen',
    departed: 'Paketzentrum verlassen',
    out_for_delivery: 'In Auslieferung'
};
...
async function handleSetup() {
    ...
    if (!apiKey) {
        alert('Bitte geben Sie Ihren JSONbin API Key ein');
        return;
    }

    saveSetupBtn.textContent = 'Verbinden...';
    ...
    try {
        ...
        alert('✅ Erfolgreich verbunden!\n\nIhre Bin ID: ' + binId + '\n\nSpeichern Sie diese ID für andere Geräte.');
    } catch (error) {
        alert('❌ Verbindung fehlgeschlagen. Bitte prüfen Sie den API Key.');
    } finally {
        saveSetupBtn.textContent = 'Speichern & Verbinden';
...
async function handleCreateShipment(e) {
    ...
    const shipment = {
        ...
        status: 'pending',
        description: description || 'Paket',
        checkpoints: [
            {
                location: origin,
                status: 'Paket empfangen',
                timestamp: new Date().toISOString(),
                notes: 'Sendung erstellt'
            }
        ],
...
async function handleDeleteShipment() {
    if (!confirm('Möchten Sie diese Sendung wirklich löschen?')) return;
...
function handleShareLink() {
    ...
    navigator.clipboard.writeText(trackingUrl).then(() => {
        ...
        prompt('Tracking-Link kopieren:', trackingUrl);
    });
}
    ...
    card.innerHTML = `
        ...
        <div class="shipment-route">
            <div class="shipment-location">
                <span class="shipment-location-label">Von</span>
            ...
            <div class="shipment-location">
                <span class="shipment-location-label">Nach</span>
...
function renderTrackingDetail(shipment) {
    ...
    if (shipment.status === 'delivered') {
        etaEl.textContent = 'Zugestellt';
    } else {
        etaEl.textContent = 'Unterwegs';
    }
