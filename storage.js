...
async function saveShipmentsToCloud(shipments) {
    ...
    try {
        // First get the latest data to ensure we don't overwrite
        const getResponse = await fetch(`${JSONBIN_API}/b/${getBinId()}/latest`, {
            headers: { 'X-Access-Key': getApiKey() }
        });

        let currentShipments = [];
        if (getResponse.ok) {
            const data = await getResponse.json();
            currentShipments = data.record.shipments || [];
        }

        // Merge logic: simpler for now, just trust the new shipments list 
        // as we are single-user focused, but in a real app we'd merge by ID
